# M1 — Broker, borgd skeleton, health, status page
...
revisit). Password file and ACL per the contract's broker table: `borgd` writes
...
- Clean shutdown, and an **LWT** so a dead borgd is visible rather than silently stale.
...
throws is a degraded capability, never a crashed borgd.

### Status page

`http://borg-pi/` served by borgd's HTTP server on port 80: one glance at what is up,
...
- Killing borgd marks it dead through the LWT; restarting it restores the picture.
