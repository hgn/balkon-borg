...
borgd skeleton, health system, audio chain — such that the Pi runs **standalone without
...
access without workarounds. The **borgd + audio run in the user session** (PipeWire
...
**D3 — borgd: one process, modular inside.** No microservices; the coupling (mode
...
Concretely, borgd keeps a **health registry**:
...
- Rendered as a **central status page at `http://borg-pi:80`** (borgd's aiohttp) —
...
| **M1** | Mosquitto quadlet (+ auth) · borgd skeleton: retained `balkon/mode/*` defaults, **health registry + status page (:80)**, user unit · deploy loop | `mosquitto_sub`, browser |
...
`src/pi/`: `provision.py` · `borgd/` (`modes/`, `audio/`, `buffers/`, `health/`,
